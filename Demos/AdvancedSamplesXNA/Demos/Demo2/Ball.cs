...
namespace FarseerGames.AdvancedSamples.Demos.Demo2
{
    public class Ball
    {
        ...
        {
            _texture = DrawingSystem.DrawingHelper.CreateCircleTexture(graphicsDevice, 12, Color.Yellow, Color.Black);
            _origin = new Vector2(_texture.Width / 2f, _texture.Height / 2f);

            _body = BodyFactory.Instance.CreateCircleBody(physicsSimulator, 12, 1);
            ...
            _geom = GeomFactory.Instance.CreateCircleGeom(physicsSimulator, _body, 12, 10, 0.5f);
