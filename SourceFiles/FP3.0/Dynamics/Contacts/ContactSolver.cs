...
namespace FarseerPhysics.Dynamics
{
            ...
            for (int i = 0; i < contactCount; ++i)
            {
                ...
                for (int j = 0; j < manifoldCount; ++j)
                {
                    ...
                    for (int k = 0; k < cc.PointCount; ++k)
                    {
                        ...
                        ccp.R1 = CommonMath.Mul(b1.GetXForm().R, cp.LocalPoint1 - b1.GetLocalCenter());
                        ccp.R2 = CommonMath.Mul(b2.GetXForm().R, cp.LocalPoint2 - b2.GetLocalCenter());

                        float rn1 = CommonMath.Cross(ref ccp.R1, ref normal);
                        float rn2 = CommonMath.Cross(ref ccp.R2, ref normal);
                        rn1 *= rn1;
                        rn2 *= rn2;
                        ...
                        Vector2 tangent = CommonMath.Cross(normal, 1.0f);

                        float rt1 = CommonMath.Cross(ref ccp.R1, ref tangent);
                        float rt2 = CommonMath.Cross(ref ccp.R2, ref tangent);
                        rt1 *= rt1;
                        rt2 *= rt2;
                        ...
                        else
                        {
                            float vRel = Vector2.Dot(cc.Normal,
                                                     v2 + CommonMath.Cross(w2, ccp.R2) - v1 -
                                                     CommonMath.Cross(w1, ccp.R1));
                    ...
                    if (cc.PointCount == 2)
                    {
                        ...
                        float rn11 = CommonMath.Cross(ref ccp1.R1, ref normal);
                        float rn12 = CommonMath.Cross(ref ccp1.R2, ref normal);
                        float rn21 = CommonMath.Cross(ref ccp2.R1, ref normal);
                        float rn22 = CommonMath.Cross(ref ccp2.R2, ref normal);
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                if (step.WarmStarting)
                {
                    for (int j = 0; j < c.PointCount; ++j)
                    {
                        ...
                        b1._angularVelocity -= invI1*CommonMath.Cross(ref ccp.R1, ref P);
                        b1._linearVelocity -= invMass1*P;
                        b2._angularVelocity += invI2*CommonMath.Cross(ref ccp.R2, ref P);
        ...
        public void SolveVelocityConstraints()
        {
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                if (c.PointCount == 1)
                {
                    ...
                    Vector2 dv = v2 + CommonMath.Cross(w2, ccp.R2) - v1 - CommonMath.Cross(w1, ccp.R1);
                    ...
                    float newImpulse = CommonMath.Max(ccp.NormalImpulse + lambda, 0.0f);
                    ...
                    v1 -= invMass1*P;
                    w1 -= invI1*CommonMath.Cross(ref ccp.R1, ref P);

                    v2 += invMass2*P;
                    w2 += invI2*CommonMath.Cross(ref ccp.R2, ref P);
                ...
                else
                {
                    ...
                    // Relative velocity at contact
                    Vector2 dv1 = v2 + CommonMath.Cross(w2, cp1.R2) - v1 - CommonMath.Cross(w1, cp1.R1);
                    Vector2 dv2 = v2 + CommonMath.Cross(w2, cp2.R2) - v1 - CommonMath.Cross(w1, cp2.R1);
                    ...
                    for (;;)
                    {
                        ...
                        // x' = - inv(A) * b'
                        //
                        Vector2 x = -CommonMath.Mul(c.NormalMass, b);
                        ...
                        if (x.X >= 0.0f && x.Y >= 0.0f)
                        {
                            ...
                            v1 -= invMass1*(P1 + P2);
                            w1 -= invI1*(CommonMath.Cross(ref cp1.R1, ref P1) + CommonMath.Cross(ref cp2.R1, ref P2));

                            v2 += invMass2*(P1 + P2);
                            w2 += invI2*(CommonMath.Cross(ref cp1.R2, ref P1) + CommonMath.Cross(ref cp2.R2, ref P2));
    ...
    // Postconditions
							...
							Box2DXDebug.Assert(Common.MathHelper.Abs(vn1 - cp1.VelocityBias) < k_errorTol);
							Box2DXDebug.Assert(Common.MathHelper.Abs(vn2 - cp2.VelocityBias) < k_errorTol);
#endif
                            break;
                        }
    ...
    // Postconditions
                ...
                for (int j = 0; j < c.PointCount; ++j)
                {
                    ...
                    float newImpulse = CommonMath.Clamp(ccp.TangentImpulse + lambda, -maxFriction, maxFriction);
        ...
        public bool SolvePositionConstraints(float baumgarte)
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                for (int j = 0; j < c.PointCount; ++j)
                {
                    ...
                    Vector2 r1 = CommonMath.Mul(b1.GetXForm().R, ccp.LocalAnchor1 - b1.GetLocalCenter());
                    Vector2 r2 = CommonMath.Mul(b2.GetXForm().R, ccp.LocalAnchor2 - b2.GetLocalCenter());
                    ...
                    minSeparation = CommonMath.Min(minSeparation, separation);

                    // Prevent large corrections and allow slop.
                    float C = baumgarte*
                              CommonMath.Clamp(separation + Settings.LinearSlop, -Settings.MaxLinearCorrection, 0.0f);
                    ...
                    b1._sweep.A -= invI1*CommonMath.Cross(ref r1, ref P);
                    b1.SynchronizeTransform();

                    b2._sweep.C += invMass2*P;
                    b2._sweep.A += invI2*CommonMath.Cross(ref r2, ref P);
