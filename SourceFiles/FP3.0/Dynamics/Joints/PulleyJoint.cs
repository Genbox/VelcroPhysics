...
namespace FarseerPhysics.Dynamics
{
	...
	public class PulleyJoint : Joint
	{
		...
		internal override void InitVelocityConstraints(TimeStep step)
		{
            ...
            float cr1u1 = Common.Math.Cross(ref r1, ref _u1);
            float cr2u2 = Common.Math.Cross(ref r2, ref _u2);
			...
			if (step.WarmStarting)
			{
				...
				b1._angularVelocity += b1._invI * Common.Math.Cross(ref r1, ref P1);
				b2._linearVelocity += b2._invMass * P2;
				b2._angularVelocity += b2._invI * Common.Math.Cross(ref r2, ref P2);
			}
			else
			{
				_impulse = 0.0f;
		...
		internal override void SolveVelocityConstraints(TimeStep step)
		{
			...
			{
                Vector2 v1 = b1._linearVelocity + Common.Math.Cross(b1._angularVelocity, r1);
                Vector2 v2 = b2._linearVelocity + Common.Math.Cross(b2._angularVelocity, r2);
		...
		internal override bool SolvePositionConstraints(float baumgarte)
		{
			...
			if (_state == LimitState.AtUpperLimit)
			{
				...
				float C = _constant - length1 - _ratio * length2;
                linearError = Common.Math.Max(linearError, -C);
                ...
                b1._sweep.A += b1._invI * Common.Math.Cross(ref r1, ref P1);
				b2._sweep.C += b2._invMass * P2;
                b2._sweep.A += b2._invI * Common.Math.Cross(ref r2, ref P2);
