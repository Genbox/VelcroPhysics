...
namespace FarseerPhysics.Dynamics
{
	...
	public class LineJoint : Joint
	{
			...
			: base(def)
		{
			...
			_localYAxis1 = FarseerPhysics.Common.Math.Cross(1.0f, _localXAxis1);
		...
		public float GetJointSpeed()
		{
			...
			Vector2 r1 = FarseerPhysics.Common.Math.Mul(b1.GetXForm().R, _localAnchor1 - b1.GetLocalCenter());
			Vector2 r2 = FarseerPhysics.Common.Math.Mul(b2.GetXForm().R, _localAnchor2 - b2.GetLocalCenter());
            ...
            float speed = Vector2.Dot(d, FarseerPhysics.Common.Math.Cross(w1, axis)) + Vector2.Dot(axis, v2 + FarseerPhysics.Common.Math.Cross(w2, r2) - v1 - FarseerPhysics.Common.Math.Cross(w1, r1));
			return speed;
		}
		...
		internal override void InitVelocityConstraints(TimeStep step)
		{
            ...
            Vector2 r1 = FarseerPhysics.Common.Math.Mul(xf1.R, _localAnchor1 - _localCenter1);
            Vector2 r2 = FarseerPhysics.Common.Math.Mul(xf2.R, _localAnchor2 - _localCenter2);
			...
			{
                _axis = FarseerPhysics.Common.Math.Mul(xf1.R, _localXAxis1);
                Vector2 temp = d + r1;
                _a1 = FarseerPhysics.Common.Math.Cross(ref temp, ref _axis);
                _a2 = FarseerPhysics.Common.Math.Cross(ref r2, ref _axis);
			...
			{
                _perp = FarseerPhysics.Common.Math.Mul(xf1.R, _localYAxis1);
                Vector2 temp2 = d + r1;
                _s1 = FarseerPhysics.Common.Math.Cross(ref temp2, ref _perp);
                _s2 = FarseerPhysics.Common.Math.Cross(ref r2, ref _perp);
			...
			if (_enableLimit)
			{
                ...
                if (FarseerPhysics.Common.Math.Abs(_upperTranslation - _lowerTranslation) < 2.0f * Settings.LinearSlop)
				{
		...
		internal override void SolveVelocityConstraints(TimeStep step)
		{
			...
			if (_enableMotor && _limitState != LimitState.EqualLimits)
			{
                ...
                _motorImpulse = FarseerPhysics.Common.Math.Clamp(_motorImpulse + impulse, -maxImpulse, maxImpulse);
			...
			if (_enableLimit && _limitState != LimitState.InactiveLimit)
			{
				...
				if (_limitState == LimitState.AtLowerLimit)
				{
                    _impulse.Y = FarseerPhysics.Common.Math.Max(_impulse.Y, 0.0f);
				}
				else if (_limitState == LimitState.AtUpperLimit)
				{
                    _impulse.Y = FarseerPhysics.Common.Math.Min(_impulse.Y, 0.0f);
				}
		...
		internal override bool SolvePositionConstraints(float baumgarte)
		{
            ...
            Vector2 r1 = FarseerPhysics.Common.Math.Mul(R1, _localAnchor1 - _localCenter1);
            Vector2 r2 = FarseerPhysics.Common.Math.Mul(R2, _localAnchor2 - _localCenter2);
			Vector2 d = c2 + r2 - c1 - r1;

			if (_enableLimit)
			{
                _axis = FarseerPhysics.Common.Math.Mul(R1, _localXAxis1);
                Vector2 temp = d + r1;
                _a1 = FarseerPhysics.Common.Math.Cross(ref temp, ref _axis);
                _a2 = FarseerPhysics.Common.Math.Cross(ref r2, ref _axis);
                ...
                if (FarseerPhysics.Common.Math.Abs(_upperTranslation - _lowerTranslation) < 2.0f * Settings.LinearSlop)
				{
                    ...
                    C2 = FarseerPhysics.Common.Math.Clamp(translation, -Settings.MaxLinearCorrection, Settings.MaxLinearCorrection);
                    linearError = FarseerPhysics.Common.Math.Abs(translation);
					active = true;
				}
				else if (translation <= _lowerTranslation)
				{
					// Prevent large linear corrections and allow some slop.
                    C2 = FarseerPhysics.Common.Math.Clamp(translation - _lowerTranslation + Settings.LinearSlop, -Settings.MaxLinearCorrection, 0.0f);
				...
				{
					// Prevent large linear corrections and allow some slop.
                    C2 = FarseerPhysics.Common.Math.Clamp(translation - _upperTranslation - Settings.LinearSlop, 0.0f, Settings.MaxLinearCorrection);
					linearError = translation - _upperTranslation;
					active = true;
				}
			}

            _perp = FarseerPhysics.Common.Math.Mul(R1, _localYAxis1);
            Vector2 temp2 = d + r1;
            _s1 = FarseerPhysics.Common.Math.Cross(ref temp2, ref _perp);
            _s2 = FarseerPhysics.Common.Math.Cross(ref r2, ref _perp);
            ...
            linearError = FarseerPhysics.Common.Math.Max(linearError, FarseerPhysics.Common.Math.Abs(C1));
			angularError = 0.0f;
