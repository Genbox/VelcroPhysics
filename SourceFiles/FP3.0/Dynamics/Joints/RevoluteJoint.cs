    ...
    public class RevoluteJoint : Joint
    {
        ...
        internal override void InitVelocityConstraints(TimeStep step)
        {
            ...
            if (_enableLimit)
            {
                ...
                if (Math.MathHelper.Abs(_upperAngle - _lowerAngle) < 2.0f*Settings.AngularSlop)
                {
            ...
            if (step.WarmStarting)
            {
                ...
                b1._angularVelocity -= i1*(Math.MathHelper.Cross(ref r1, ref P) + _motorImpulse + _impulse.Z);

                b2._linearVelocity += m2*P;
                b2._angularVelocity += i2*(Math.MathHelper.Cross(ref r2, ref P) + _motorImpulse + _impulse.Z);
            }
            else
            {
        ...
        internal override void SolveVelocityConstraints(TimeStep step)
        {
            ...
            if (_enableLimit && _limitState != LimitState.InactiveLimit)
            {
                ...
                Vector2 Cdot1 = v2 + Math.MathHelper.Cross(w2, r2) - v1 - Math.MathHelper.Cross(w1, r1);
                float Cdot2 = w2 - w1;
                ...
                v1 -= m1*P;
                w1 -= i1*(Math.MathHelper.Cross(ref r1, ref P) + impulse.Z);

                v2 += m2*P;
                w2 += i2*(Math.MathHelper.Cross(ref r2, ref P) + impulse.Z);
            }
            else
            {
                Vector2 r1 = Math.MathHelper.Mul(b1.GetXForm().R, _localAnchor1 - b1.GetLocalCenter());
                ...
                // Solve point-to-point constraint
                Vector2 Cdot = v2 + Math.MathHelper.Cross(w2, r2) - v1 - Math.MathHelper.Cross(w1, r1);
                ...
                v1 -= m1*impulse;
                w1 -= i1*Math.MathHelper.Cross(ref r1, ref impulse);

                v2 += m2*impulse;
                w2 += i2*Math.MathHelper.Cross(ref r2, ref impulse);
            }
        ...
        internal override bool SolvePositionConstraints(float baumgarte)
        {
            ...
            if (_enableLimit && _limitState != LimitState.InactiveLimit)
            {
                ...
                if (_limitState == LimitState.EqualLimits)
                {
                    ...
                    float C = Math.MathHelper.Clamp(angle, -Settings.MaxAngularCorrection, Settings.MaxAngularCorrection);
                    limitImpulse = -_motorMass*C;
                    angularError = Math.MathHelper.Abs(C);
                }
                else if (_limitState == LimitState.AtLowerLimit)
                {
                    ...
                    C = Math.MathHelper.Clamp(C + Settings.AngularSlop, -Settings.MaxAngularCorrection, 0.0f);
                    limitImpulse = -_motorMass*C;
                }
                else if (_limitState == LimitState.AtUpperLimit)
                {
                    ...
                    // Prevent large angular corrections and allow some slop.
                    C = Math.MathHelper.Clamp(C - Settings.AngularSlop, 0.0f, Settings.MaxAngularCorrection);
                    limitImpulse = -_motorMass*C;
                }
            ...
            // Solve point-to-point constraint.
            {
                ...
                b1._sweep.A -= b1._invI*Math.MathHelper.Cross(ref r1, ref impulse_);

                b2._sweep.C += b2._invMass*impulse_;
                b2._sweep.A += b2._invI*Math.MathHelper.Cross(ref r2, ref impulse_);
