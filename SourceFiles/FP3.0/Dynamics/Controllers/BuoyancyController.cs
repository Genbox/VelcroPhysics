	...
	public class BuoyancyController : Controller
	{
		...
		public override void Step(TimeStep step)
		{
			...
			for (ControllerEdge i = _bodyList; i != null; i = i.nextBody)
			{
				...
				massc.X /= mass;
				massc.Y /= mass;
				if (area < FarseerPhysics.Common.Settings.FLT_EPSILON)
					continue;
				//Buoyancy
		...
		{
			float r = 1000;
			Vector2 p1 = offset * normal + FarseerPhysics.Common.Math.Cross(normal, r);
            Vector2 p2 = offset * normal - FarseerPhysics.Common.Math.Cross(normal, r);
