...
namespace FarseerPhysics.Dynamics
{
    ...
    public class PhysicsSimulator : IDisposable
    {
        ...
        public PhysicsSimulator(AABB worldAABB, Vector2 gravity, bool doSleep)
        {
        ...
        {
            return _broadPhase._proxyCount;
        }
        ...
        {
            return _broadPhase._pairManager._pairCount;
        }
        ...
        public Shape RaycastOne(Segment segment, out float lambda, out Vector2 normal, bool solidShapes, object userData)
        {
            lambda = 0;
        ...
        {
            // Step all controlls
            for (Controller controller = _controllerList; controller != null; controller = controller._next)
            {
                ...
                for (Body seed = _bodyList; seed != null; seed = seed._next)
                {
                    ...
                    while (stackCount > 0)
                    {
                        ...
                        for (ContactEdge cn = b._contactList; cn != null; cn = cn.Next)
                        {
                            ...
                            if ((cn.Contact._flags & (Contact.CollisionFlags.Island | Contact.CollisionFlags.NonSolid)) !=
                                0)
                            {
                                continue;
                            }
            ...
            for (;;)
            {
                ...
                for (Contact c = _contactList; c != null; c = c._next)
                {
                    ...
                    else
                    {
                        ...
                        {
                            toi = MathHelper.Min((1.0f - toi)*t0 + toi, 1.0f);
                        }
                ...
                while (queueSize > 0)
                {
                    ...
                    for (ContactEdge cn = b._contactList; cn != null; cn = cn.Next)
                    {
                        ...
                        if ((cn.Contact._flags &
                             (Contact.CollisionFlags.Island | Contact.CollisionFlags.Slow |
                ...
                case ShapeType.CircleShape:
                    {
                        ...
                        Vector2 center = MathHelper.Mul(xf, circle.GetLocalPosition());
                ...
                case ShapeType.PolygonShape:
                    {
                        ...
                        {
                            vertices[i] = MathHelper.Mul(xf, localVertices[i]);
                        }
                        ...
                        if (core)
                        {
                            ...
                            for (int i = 0; i < vertexCount; ++i)
                            {
                                vertices[i] = MathHelper.Mul(xf, localCoreVertices[i]);
                            }
            ...
            if ((flags & DebugDraw.DrawFlags.Obb) != 0)
            {
                ...
                for (Body b = _bodyList; b != null; b = b.GetNext())
                {
                    ...
                    for (Shape s = b.GetShapeList(); s != null; s = s.GetNext())
                    {
                        ...
                        Vector2[] vs = new Vector2[4];
                        vs[0] = new Vector2(-h.X, -h.Y);
                        vs[1] = new Vector2(h.X, -h.Y);
                        vs[2] = new Vector2(h.X, h.Y);
                        vs[3] = new Vector2(-h.X, h.Y);

                        for (int i = 0; i < 4; ++i)
                        {
                            vs[i] = obb.Center + MathHelper.Mul(obb.R, vs[i]);
                            vs[i] = MathHelper.Mul(xf, vs[i]);
                        }
        ...
        private static float RaycastSortKey(object data)
        {
            ...
            PhysicsSimulator world = body.GetWorld();
            ...
            float lambda;
            SegmentCollide collide = shape.TestSegment(xf, out lambda, out world._raycastNormal, world._raycastSegment,
                                                       1);
