...
#warning "CAS"
    ...
    public class BroadPhase
    {
        ...
        public BroadPhase(AABB worldAABB, PairCallback callback)
        {
            ...
            //This code is only in the C# version to fill the bounds array with Bound objects.
            for (int i = 0; i < 2; i++)
            {
        ...
        public ushort CreateProxy(AABB aabb, object userData)
        {
            ...
            ushort[] lowerValues, upperValues;
...
#warning "Check this"
                ...
                for (int index = lowerIndex; index < boundCount + 2; ++index)
                {
                    ...
                    {
                        _proxyPool[bounds[index].ProxyId].LowerBounds[axis] = (ushort)index;
                    }
                    else
                    {
                        _proxyPool[bounds[index].ProxyId].UpperBounds[axis] = (ushort)index;
                    }
                }
            }
...
#warning "Check this"
            ...
            for (int axis = 0; axis < 2; ++axis)
            {
                ...
                if (deltaLower < 0)
                {
                    ...
                    while (index > 0 && lowerValue < bounds[index - 1].Value)
                    {
                        ...
                        int prevProxyId = bounds[index - 1].ProxyId;

                        ++bounds[index - 1].StabbingCount;

                        if (bounds[index - 1].IsUpper == true)
                        {
                            if (TestOverlap(newValues, _proxyPool[prevProxyId]))
                            {
                ...
                if (deltaUpper > 0)
                {
                    ...
                    {
                        Bound bound = bounds[index];
                        int nextProxyId = bounds[index + 1].ProxyId;

                        ++bounds[index + 1].StabbingCount;

                        if (bounds[index + 1].IsLower == true)
                        {
        ...
        private bool TestOverlap(BoundValues b, Proxy p)
        {
            for (int axis = 0; axis < 2; ++axis)
            {
                Bound[] bounds = _bounds[axis];
        ...
        private void IncrementTimeStamp()
        {
            ...
            {
#warning "Ushort used here, but Settings.MaxProxies is an int"
