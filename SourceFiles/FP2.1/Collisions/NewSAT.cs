...
{
    public class NewSAT : INarrowPhaseCollider
    {
        private static NewSAT _instance;

        private NewSAT()
        {
        }

        public static NewSAT Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new NewSAT();
                }
                return _instance;
            }
        }
                    ...
                    if (InsidePolygon(geomB.WorldVertices, geomA.WorldVertices[i]))
                    {
                        //if (!geomA.Body.IsStatic)
                        {
                            if (distanceA > 0.001f)
                            {
                                ...
                                Contact c = new Contact(_tempVector, normalA, -distanceB, _tempContactId);
                                contactList.Add(c);
                                contactsDetected++;
                            }
                        }
