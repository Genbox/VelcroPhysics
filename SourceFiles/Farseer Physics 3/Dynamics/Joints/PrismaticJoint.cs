	...
	public class PrismaticJoint : Joint
	{
		...
		public float JointSpeed
		{
			get
			{
				...
				float speed = Vector2.Dot(d, Common.Math.Cross(w1, axis)) + Vector2.Dot(axis, v2 + Common.Math.Cross(w2, r2) - v1 - Common.Math.Cross(w1, r1));
				return speed;
			}
		}
			...
			: base(def)
		{
			...
			_localYAxis1 = Common.Math.Cross(1.0f, _localXAxis1);
		...
		internal override void InitVelocityConstraints(TimeStep step)
		{
			...
			Vector2 r1 = Common.Math.Mul(xf1.R, _localAnchor1 - _localCenter1);
			Vector2 r2 = Common.Math.Mul(xf2.R, _localAnchor2 - _localCenter2);
			...
			{
				_axis = Common.Math.Mul(xf1.R, _localXAxis1);
                Vector2 temp = d + r1;
                _a1 = Common.Math.Cross(ref temp, ref _axis);
                _a2 = Common.Math.Cross(ref r2, ref _axis);
			...
			{
				_perp = Common.Math.Mul(xf1.R, _localYAxis1);
                Vector2 temp = d + r1;
                _s1 = Common.Math.Cross(ref temp, ref _perp);
                _s2 = Common.Math.Cross(ref r2, ref _perp);
		...
		internal override void SolveVelocityConstraints(TimeStep step)
		{
			...
			if (_enableLimit && _limitState != LimitState.InactiveLimit)
			{
				...
				if (_limitState ==LimitState.AtLowerLimit)
				{
					_impulse.Z = Common.Math.Max(_impulse.Z, 0.0f);
				}
				else if (_limitState == LimitState.AtUpperLimit)
				{
					_impulse.Z = Common.Math.Min(_impulse.Z, 0.0f);
				}
		...
		internal override bool SolvePositionConstraints(float baumgarte)
		{
            ...
            Vector2 temp2 = d + r1;
            _s1 = Common.Math.Cross(ref temp2, ref _perp);
            _s2 = Common.Math.Cross(ref r2, ref _perp);
