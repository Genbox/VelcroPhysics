	...
	public class PolygonShape : Shape, Collision.IGenericShape
	{
			...
			: base(def)
		{
			...
			for (int i = 0; i < _vertexCount; ++i)
			{
                ...
                _normals[i] = Common.Math.Cross(edge, 1.0f);
			...
			for (int i = 1; i < _vertexCount; ++i)
			{
                float cross = Common.Math.Cross(ref _normals[i - 1], ref _normals[i]);
		...
		public override void ComputeMass(out MassData massData)
		{
			...
			for (int i = 0; i < _vertexCount; ++i)
			{
                ...
                float D = Common.Math.Cross(ref e1, ref e2);
        ...
        public override float ComputeSubmergedArea(Vector2 normal, float offset, XForm xf, out Vector2 c)
		{
            ...
            c = new Vector2();
            Vector2 normalL = FarseerPhysics.Common.Math.MulT(xf.R, normal);
			...
			switch (diveCount)
			{
				case 0:
					if (lastSubmerged)
					{
						...
						MassData md;
						ComputeMass(out md);
                        c = FarseerPhysics.Common.Math.Mul(xf, md.Center);
			...
			c = FarseerPhysics.Common.Math.Mul(xf, center);
        ...
        public static void ComputeOBB(out OBB obb, Vector2[] vs, int count)
		{
			...
			for (int i = 1; i <= count; ++i)
			{
                Vector2 root = p[i - 1];
                Vector2 ux = p[i] - root;
                float length = Common.Math.Normalize(ref ux);
