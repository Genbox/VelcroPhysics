...
namespace FarseerPhysics.Common
{
    public class PolygonBrush
    {
        ...
        public void Draw(ref GraphicsDevice device)
        {
            if (filled)
            {
                device.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.TriangleFan, vertices,
                    0, segments - 1);
                for (int i = 0; i < segments + 1; i++)
                {
                    vertices[i].Color = Color.Black;
                }
                device.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.LineStrip, vertices,
                    0, segments);
            }
            else
                device.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.LineStrip, vertices,
    ...
    public class CircleBrushDefinition
    {
        ...
        private CircleBrushDefinition()
        {
            ...
            for (int i = 0; i < Segments; ++i)
            {
                Vertices[i] = 1.0f * new Vector2((float)System.Math.Cos(theta), (float)System.Math.Sin(theta));
    ...
    public class CircleBrush
    {
        ...
        public void Draw(ref GraphicsDevice device)
        {
            ...
            {
                verts[i] = new VertexPositionColor(new Vector3((CircleBrushDefinition.Instance.Vertices[i] * Radius) + Position, 0.0f), Color);
            }

            verts[CircleBrushDefinition.Instance.Segments] = new VertexPositionColor(new Vector3((CircleBrushDefinition.Instance.Vertices[0] * Radius) + Position, 0.0f), Color);

            if (Filled)
            {
                device.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.TriangleFan, verts,
                ...
                device.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.LineStrip, verts,
    ...
    public class XNADebugRenderer : DebugDraw 
    {
        ...
        private List<VertexPositionColor> GetTriangleStrip(Vector3[] points, Color color, float thickness)
        {
            ...
            _normal.Normalize();
            Vector3 _p1 = lastPoint + _normal * thickness; triangleCount++;
            Vector3 _p2 = lastPoint - _normal * thickness; triangleCount++;
            Vector3 _p3 = points[0] + _normal * thickness; triangleCount++;
            Vector3 _p4 = points[0] - _normal * thickness; triangleCount++;
