...
namespace FarseerPhysics.Samples.Demos.Prefabs
{
  public class Spiderweb
  {
    ...
    public void LoadContent(ContentManager content)
    {
      ...
      _spriteScale = 2f * ConvertUnits.ToDisplayUnits(_radius) / _goo.Image.Width;
    }
    ...
    public void Draw(SpriteBatch batch)
    {
      foreach (Joint j in _world.JointList)
      {
        if (j.Enabled && j.JointType != JointType.FixedMouse)
        {
          ...
          Vector2 scale = new Vector2(distance / _link.Image.Width, _spriteScale);
      ...
      foreach (Body b in _world.BodyList)
      {
        ...
        {
          batch.Draw(_goo.Image, ConvertUnits.ToDisplayUnits(b.Position), null,
