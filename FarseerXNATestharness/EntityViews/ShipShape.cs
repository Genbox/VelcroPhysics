...
namespace FarseerGames.FarseerXNATestharness.EntityViews {
    class ShipShape : PolygonShape, IEntityView {
        float _width;
        float _height;

        public ShipShape(Game game, float width, float height)
            : base(game) {
        ...
        protected override void FillBuffer(VertexPositionColor[] data) {
            data[0] = new VertexPositionColor(new Vector3(0f, _height*.5f, 0f), Color.White);
            data[1] = new VertexPositionColor(new Vector3(_width * .5f, -_height * .5f, 0f), Color.White);
            data[2] = data[1];
            data[3] = new VertexPositionColor(new Vector3(0f, -_height*.25f, 0f), Color.White);
            data[4] = data[3];
